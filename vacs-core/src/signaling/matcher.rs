...
struct MatcherEntry {
    predicate: Box<dyn Fn(&Message) -> bool + Send + Sync + 'static>,
...
impl ResponseMatcher {
    ...
    /// - `Err(SignalingError:Disconnected)` if the Matcher was closed unexpectedly.
    #[instrument(level = "debug", skip(self, predicate), err)]
    pub async fn wait_for<F>(&self, predicate: F) -> Result<Message, SignalingError>
    where
        F: Fn(&Message) -> bool + Send + Sync + 'static,
    {
    ...
    pub fn try_match(&self, msg: &Message) {
...
mod tests {
    use super::*;
    use crate::signaling::ClientInfo;
    ...
    async fn wait_for() {
        ...
        let handle = tokio::spawn(async move {
            matcher_clone
                .wait_for(|msg| matches!(msg, Message::Logout))
                .await
        });

        tokio::time::sleep(Duration::from_millis(10)).await;
        matcher.try_match(&Message::Logout);

        let result = handle.await.unwrap();
        assert_matches!(result, Ok(Message::Logout));
    }
    ...
    async fn wait_for_content() {
        ...
        let handle = tokio::spawn(async move {
            matcher_clone
                .wait_for(|msg| matches!(msg, Message::ClientList { .. }))
                .await
        });
        ...
        let result = handle.await.unwrap();
        assert_matches!(result, Ok(Message::ClientList { clients }) if clients.len() == 1);
    }
    ...
    async fn wait_for_matching_peer_id() {
        ...
        let handle = tokio::spawn(async move {
            matcher_clone
                .wait_for(|msg| matches!(msg, Message::CallAnswer { peer_id, .. } if peer_id == "client2"))
                .await
        });
        ...
        let result = handle.await.unwrap();
        assert_matches!(result, Ok(Message::CallAnswer { peer_id, sdp }) if peer_id == "client2" && sdp == "sdp2");
    }
        ...
        let handle = tokio::spawn(async move {
            matcher_clone
                .wait_for_with_timeout(
                    |msg| matches!(msg, Message::Logout),
    ...
    async fn try_match_matches_only_once() {
        ...
        let h1 = tokio::spawn(async move {
            m1.wait_for_with_timeout(|m| matches!(m, Message::Logout), Duration::from_millis(20))
                .await
        });
        let h2 = tokio::spawn(async move {
            m2.wait_for_with_timeout(|m| matches!(m, Message::Logout), Duration::from_millis(20))
                .await
        });
        ...
        assert!(matches!(r1, Ok(Message::Logout)) ^ matches!(r2, Ok(Message::Logout)));
    }
    ...
    async fn try_match_with_overlapping_predicates() {
        ...
        let h1 = tokio::spawn(async move {
            m1.wait_for_with_timeout(
                |m| matches!(m, Message::Logout | Message::PeerNotFound { .. }),
        ...
        let matches = [r1, r2]
            .iter()
            .filter(|r| matches!(r, Ok(Message::Logout)))
            .count();
        assert_eq!(matches, 1);
    }
    ...
    async fn try_match_concurrent_waiters() {
        ...
        let mut matches = 0;
        for h in handles {
            if matches!(h.await.unwrap(), Ok(Message::Logout)) {
                matches += 1;
            }
        }
        assert_eq!(matches, 1);
    ...
    async fn try_match_burst() {
        ...
        let w1 = tokio::spawn(async move {
            h1.wait_for(|msg| matches!(msg, Message::CallAnswer { .. }))
                .await
        });

        let w2 = tokio::spawn(async move {
            h2.wait_for(|msg| matches!(msg, Message::ClientList { .. }))
                .await
        });
        ...
        matcher.try_match(&Message::ClientList {
            clients: vec![ClientInfo {
                id: "client1".into(),
                display_name: "Client 1".into(),
            }],
        });
        matcher.try_match(&Message::CallAnswer {
            peer_id: "client2".to_string(),
            sdp: "sdp2".into(),
        });
        ...
        assert_matches!(r1, Ok(Message::CallAnswer { .. }));
        assert_matches!(r2, Ok(Message::ClientList { .. }));
    }
